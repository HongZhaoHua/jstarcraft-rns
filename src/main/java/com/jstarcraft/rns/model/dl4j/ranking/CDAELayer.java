...
public class CDAELayer extends BaseLayer<CDAEConfiguration> {
    ...
    public Pair<Gradient, INDArray> backpropGradient(INDArray epsilon, LayerWorkspaceMgr workspaceMgr) {
        ...
        // equivalent)
        INDArray z = this.preOutput(true, workspaceMgr); // Note: using preOutput(INDArray)
    ...
    public double calcL2(boolean backpropParamsOnly) {
        double l2Sum = 0.0D;
        double l2Norm;
        if (this.conf.getLayer().getL2ByParam(CDAEParameter.WEIGHT_KEY) > 0.0D) {
            l2Norm = this.getParam(CDAEParameter.WEIGHT_KEY).norm2Number().doubleValue();
            l2Sum += 0.5D * this.conf.getLayer().getL2ByParam(CDAEParameter.WEIGHT_KEY) * l2Norm * l2Norm;
        }
        if (this.conf.getLayer().getL2ByParam(CDAEParameter.USER_KEY) > 0.0D) {
            l2Norm = this.getParam(CDAEParameter.USER_KEY).norm2Number().doubleValue();
            l2Sum += 0.5D * this.conf.getLayer().getL2ByParam(CDAEParameter.USER_KEY) * l2Norm * l2Norm;
        }
        if (this.conf.getLayer().getL2ByParam(CDAEParameter.BIAS_KEY) > 0.0D) {
            l2Norm = this.getParam(CDAEParameter.BIAS_KEY).norm2Number().doubleValue();
            l2Sum += 0.5D * this.conf.getLayer().getL2ByParam(CDAEParameter.BIAS_KEY) * l2Norm * l2Norm;
        }
        return l2Sum;
    }
